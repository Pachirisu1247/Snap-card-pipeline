...
# ---------- CONFIG ----------
PORT = 5000
CARDS_CSV = "snap_cards.cleaned.csv"
TEMPLATE_PSD = "AgathaNew.psd"
OUTPUT_DIR = "output_psd"
BATCH_LIMIT = 3   # e.g., 5 to test on first 5 cards, or None for all

# Logos on disk (raw and cropped)
LOGO_DIR_RAW = r"C:\Users\allda\OneDrive - University of Southern California\Marvel Snap Cards\logos_upscaled\Logos"
...
def ensure_cropped_logos():
    ...
    and write them into LOGO_DIR (cropped). This runs on startup.
    ...
    for fname in os.listdir(LOGO_DIR_RAW):
        ...
        try:
            ...
            # use alpha channel to find non-transparent bounding box
            alpha = img.split()[3]
            bbox = alpha.getbbox()
            if bbox is None:
                # completely transparent? just copy as-is
                img.save(dst)
            else:
                cropped = img.crop(bbox)
                cropped.save(dst)
            print(f"Cropped logo: {fname}")
